...
export function MainDiagnosisCard({
  ...
  const mainCondition =
    ...
    "N/A";
  const cid =
    displayFields.find((f) => f.label.toLowerCase().includes("cid"))?.value ||
    data.cid;
  const justification = data.content || data.justification || "";

  return (
    <div
      className={`rounded-2xl border ${styles.border} bg-gradient-to-br ${styles.bg} to-white p-6 ring-1 ${styles.border}/50`}
    >
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {displayFields
                  ...
                  .map((field, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-sm font-medium text-gray-600 shadow-sm ring-1 ring-gray-200"
                    >
                      {field.label.toLowerCase().includes("confiança") ||
                      field.label.toLowerCase().includes("confidence") ? (
                        <ActivityIcon className="h-4 w-4 text-emerald-500" />
                      ) : field.label.toLowerCase().includes("severidade") ||
                        field.label.toLowerCase().includes("severity") ? (
                        <AlertIcon className="h-4 w-4 text-amber-500" />
                      ) : field.label.toLowerCase().includes("evolução") ||
                        field.label.toLowerCase().includes("evolution") ? (
      ...
      {justification && (
        <div
          className={`mt-6 rounded-xl border ${styles.border} bg-white/60 p-5`}
        >
          <h3
