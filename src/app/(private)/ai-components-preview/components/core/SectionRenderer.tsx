...
export function SectionRenderer({
  ...
  const compactCount = gridComponents.filter((c) => getComponentSize(c.type) === "compact").length;
  ...
  // - Mix ou wide: até 2 colunas com min 320px
  ...
  if (total === 0) {
    ...
    // Sem grid, respeita a largura natural do card
    gridStyle = { display: "block" };
  } else if (wideCount === 0) {
    // Apenas compactos — pode ter até 3 colunas
    const maxCols = total >= 6 ? 3 : total >= 3 ? 3 : 2;
    gridStyle = {
      display: "grid",
