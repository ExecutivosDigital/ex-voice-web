...
function CheckoutInner() {
  ...
  async function submitPix(): Promise<{ status: number; body?: any }> {
    ...
    const resp = await PostAPI(`/signature/pix/${selectedPlan.id}`, body, true);
    if ([200, 201].includes(resp.status)) {
      if (isFree) {
        await handleGetAvailableRecording();
        setIsSuccess(true);
...
function MethodPill({
  ...
  return (
    ...
    >
      ...
      {active && <Check size={14} className="ml-auto" strokeWidth={2.5} />}
    </button>
  );
}
    ...
    <div className={cn("flex flex-col gap-1.5", className)}>
      ...
      <div
        className={cn(
          ...
          focused ? "border-gray-400 ring-2 ring-gray-100" : "border-gray-200",
        )}
      >
...
function SuccessScreen({ onHome }: { onHome: () => void }) {
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute -right-20 -bottom-20 h-56 w-56 rounded-full bg-gradient-to-br from-amber-300/30 to-transparent blur-3xl" />
      ...
      <div className="relative flex flex-col gap-2">
        ...
        <h2 className="text-3xl font-semibold text-balance text-gray-900 md:text-4xl">
          Sua assinatura está{" "}
