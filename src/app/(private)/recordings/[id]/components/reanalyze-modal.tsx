...
/**
 * Re-análise com outra IA (trilha IA): regenera os 3 resumos a partir da
 * transcrição existente — sem re-transcrever. Pode mover a gravação de
 * departamento (a IA nova define o contexto). Só o dono vê o botão.
 */
...
export function ReanalyzeModal({
  ...
  async function handleReanalyze() {
    ...
    if (response.status === 200 && response.body?.ok !== false) {
      toast.success("Re-análise concluída — recarregando...");
    ...
    <Dialog open={open} onOpenChange={(o) => !o && !running && onClose()}>
      <DialogContent className="max-w-md bg-white">
        <DialogHeader>
          ...
          <DialogDescription>
            Os resumos e análises são regenerados a partir da transcrição
            existente (sem transcrever de novo). Os textos atuais serão
            substituídos.
          </DialogDescription>
        </DialogHeader>
