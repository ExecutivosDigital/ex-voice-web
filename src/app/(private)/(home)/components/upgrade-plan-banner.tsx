...
export function UpgradePlanBanner({ className }: UpgradePlanBannerProps) {
  ...
  console.log("availableRecording", availableRecording);
  console.log("totalRecording", totalRecording);
  const isExpired =
  ...
  const config = isTrial
    ? {
        ...
        titleHighlight: "do EX Voice",
        subtitle:
          "Você está no período de teste. Faça upgrade agora e tenha acesso ilimitado a gravações, relatórios inteligentes e muito mais.",
        cta: "Fazer Upgrade",
        features: [
    ...
    : {
        ...
        titleHighlight: "e continue crescendo",
        subtitle:
          "Seu plano expirou e seus recursos estão limitados. Renove agora para recuperar todo o acesso à plataforma.",
        cta: "Renovar Agora",
        features: [
          "Recupere seu acesso",
          "Mantenha seus dados",
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative cursor-pointer overflow-hidden rounded-2xl border border-white/10 p-6 shadow-xl transition-all duration-500 hover:shadow-2xl hover:shadow-neutral-900/20 sm:p-8",
        className,
      )}
      style={{
        background:
          "linear-gradient(135deg, #1a1a1a 0%, #2a2a2b 30%, #3a3b3c 60%, #1a1a1a 100%)",
      }}
    >
      ...
      <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:gap-8">
        ...
        <div className="shrink-0">
          <motion.div
            ...
            className="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/10 bg-white/5 shadow-lg backdrop-blur-sm sm:h-20 sm:w-20"
          >
            <Icon className="h-8 w-8 text-neutral-400 sm:h-10 sm:w-10" />
          </motion.div>
        </div>
        ...
        <div className="shrink-0 sm:self-center">
          <motion.div
            ...
            whileTap={{ scale: 0.97 }}
            className="bg-primary shadow-primary/25 group-hover:shadow-primary/30 flex items-center gap-2 rounded-xl px-6 py-3.5 text-sm font-bold text-white shadow-lg transition-all duration-300 group-hover:shadow-xl sm:px-8 sm:py-4 sm:text-base"
          >
            {config.cta}
            <motion.div
              animate={{ x: [0, 4, 0] }}
      ...
      <div className="via-primary/50 absolute right-0 bottom-0 left-0 h-px bg-linear-to-r from-transparent to-transparent" />
    </motion.div>
  );
}
